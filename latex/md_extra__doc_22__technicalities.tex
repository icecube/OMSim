...
\label{md_extra__doc_22__technicalities_autotoc_md31}%
\Hypertarget{md_extra__doc_22__technicalities_autotoc_md31}%
\hypertarget{md_extra__doc_22__technicalities_autotoc_md32}{}\doxysection{\texorpdfstring{Visualization of Complex Objects}{Visualization of Complex Objects}}\label{md_extra__doc_22__technicalities_autotoc_md32}
...
\end{DoxyItemize}
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_extra__doc_22__technicalities_autotoc_md33}{}\doxysection{\texorpdfstring{The Tools namespace}{The Tools namespace}}\label{md_extra__doc_22__technicalities_autotoc_md33}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{md_extra__doc_22__technicalities_autotoc_md35}{}\doxysection{\texorpdfstring{Nomenclature}{Nomenclature}}\label{md_extra__doc_22__technicalities_autotoc_md35}
...
If everything looks good, save the file (in the example above {\ttfamily m\+DOM\+\_\+\+Hamamatsu\+\_\+\+R15458\+\_\+\+CT\+\_\+intrinsic\+\_\+\+QE.\+dat}) in {\ttfamily common/data/\+PMTs/measurement\+\_\+matching\+\_\+data/\+QE/} and move to step 2.\hypertarget{md_extra__doc_22__technicalities_autotoc_md39}{}\doxysubsection{\texorpdfstring{Step 2\+: Expand OMSim\+PMTResponse and Verify QE}{Step 2: Expand OMSimPMTResponse and Verify QE}}\label{md_extra__doc_22__technicalities_autotoc_md39}
...
\end{DoxyCode}
   ...
   ~\newline
  \hypertarget{md_extra__doc_22__technicalities_autotoc_md40}{}\doxysubsection{\texorpdfstring{Step 3\+: Matching detection efficiency scan}{Step 3: Matching detection efficiency scan}}\label{md_extra__doc_22__technicalities_autotoc_md40}
...
\end{DoxyItemize}\hypertarget{md_extra__doc_22__technicalities_autotoc_md41}{}\doxysubsection{\texorpdfstring{Step 4\+: Matching gain / transit time scans}{Step 4: Matching gain / transit time scans}}\label{md_extra__doc_22__technicalities_autotoc_md41}
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{md_extra__doc_22__technicalities_autotoc_md43}{}\doxysection{\texorpdfstring{CAD geometries}{CAD geometries}}\label{md_extra__doc_22__technicalities_autotoc_md43}

\begin{DoxyItemize}
\item {\bfseries{Creating new geometries}}
\begin{DoxyItemize}
\item Use the CAD program of your choice to load/modify geometries. Module geometries are available in Share\+Point, Docushare or Google Drive.
\item Take note of the origin of the part in respect to the module center.
\item Simplify geometries by removing/editing parts unlikely to affect detector response to minimize compile times.
\item Split complex geometries into separate assemblies for flexibility (e.\+g., harness into waistband and ropes).
\item Note\+: Non-\/touching volumes in (sub)geometries will generate separate meshes.
\item Export geometry as {\ttfamily .obj} format.
\item Store in {\ttfamily common/data/\+CADmeshes} with descriptive naming. The {\ttfamily .mtl} file can be omitted.
\end{DoxyItemize}
\item {\bfseries{Implementing CAD geometries}}
\begin{DoxyItemize}
\item Implementation uses \href{https://github.com/christopherpoole/CADMesh}{\texttt{ CADMesh}} via {\ttfamily OMSim\+Tools\+::\+Append\+CADComponent}.
\item Multiple bodies in geometry generate separate meshes, requiring if-\/clauses and resulting in Multi\+Union when not visualized.
\item Example implementation\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{Tools::AppendCADComponent(\textcolor{keyword}{this},\ 1.0,\ lOrigin,\ lRotation,}
\DoxyCodeLine{\ \ \ \ \textcolor{stringliteral}{"{}DEGG/Internal\_OnlyCones.obj"{}},\ \textcolor{stringliteral}{"{}CAD\_Internal"{}},}
\DoxyCodeLine{\ \ \ \ m\_data-\/>getMaterial(\textcolor{stringliteral}{"{}NoOptic\_Absorber"{}}),\ m\_aluVis);}

\end{DoxyCode}

\item Ensure correct volume placement.
\end{DoxyItemize}
\item {\bfseries{Overlap checking}}
\begin{DoxyItemize}
\item Run simulation with {\ttfamily -\/-\/check\+\_\+overlaps} flag and verify output...all volumes should report \textquotesingle{}OK!\textquotesingle{}.
\end{DoxyItemize}
\end{DoxyItemize}

Launch with visualization to verify placement\+: {\ttfamily ./\+OMSim\+\_\+effective\+\_\+area -\/-\/detector\+\_\+type 6 -\/-\/place\+\_\+harness -\/v}
\begin{DoxyItemize}
\item Example Open\+GL viewer commands for inspection\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{/vis/viewer/addCutawayPlane\ 0\ 0\ 0\ mm\ 1\ 0\ 0}
\DoxyCodeLine{/vis/scene/add/axes\ 0\ 0\ 0\ 250\ mm}
\DoxyCodeLine{/vis/scene/add/arrow\ 0\ 0\ 0\ 0\ 90\ 40\ mm}

\end{DoxyCode}

\item Verify proper component placement in scene tree (internal components inside module, harness in world volume)
\item For overlaps\+: verify origin/rotation, check CAD vs \doxylink{class_o_m_sim}{OMSim} differences (and track them!), consider thinning CAD geometry.
\end{DoxyItemize}

{\bfseries{Verification}}
\begin{DoxyItemize}
\item Double-\/check for overlaps.
\item Test photon interactions with new geometry. This will depend on the material properties that you assigned.
\item Optionally compare effective area scans with/without new geometry
\end{DoxyItemize}
