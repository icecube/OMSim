  ...
  : G4VDiscreteProcess(processName, ptype)
{
  ...
  fNumWarnings = 0;
  G4cout << "::::::::::::::::::::::::::::::::finish constructor" << G4endl;
}
...
//Checks if the photon is at the geometry boundary. If true, it retrieves the materials of the two volumes on either side of the boundary.
  ...
  G4cout << "::::::::::::::::: PostStep 2" << G4endl;
  if(MPT != nullptr)
  {
    rIndexMPV = MPT->GetProperty(kRINDEX);
    pp = MPT->GetProperty(kABSLENGTH);
  }
  ...
  if (pp != nullptr)
  {
    fAbsorptionLength1 = pp->Value(fPhotonMomentum, idx_abslength1); // Retrieves absorption length of Material1 at the given photon momentum
  }
  // end of newly added
  else
  {
  ...
  G4cout << "::::::::::blahPostStepDoIt3" << G4endl;
                                              ...
                                              G4ForceCondition* condition)
{
  *condition = Forced;
  return DBL_MAX;
      G4cout << "blahMeanFree" << G4endl;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4OpBoundaryProcess::GetIncidentAngle()
{
        G4cout << "blahIncident" << G4endl;
                                              ...
                                              G4double imaginaryRindex)
{
  ...
  G4MaterialPropertyVector* ppI  = MPT->GetProperty(kIMAGINARYRINDEX);
        G4cout << "blahGetReflectivity" << G4endl;
...
void G4OpBoundaryProcess::CalculateReflectivity()
{
  ...
  G4double imaginaryRindex =
    fImagRIndexMPV->Value(fPhotonMomentum, idx_irindex);
      G4cout << "blahCalcReflectivity" << G4endl;
...
{
 // todo: when photon absorbed in photocathode: fStatus = Detection;
    G4cout << "blahPhotocathode" << G4endl;
    G4cout << " " << G4endl;
    G4cout << fMaterial1->GetName() << G4endl;
    G4cout << fMaterial2->GetName() << G4endl;
    G4cout << " " << G4endl;
  G4MaterialPropertyVector* pp = nullptr; // pp = a pointer

  G4Material *lPhotocathodeMaterial = G4Material::GetMaterial("RiAbs_NicoPhotCat");
  G4MaterialPropertiesTable* lMPT_photocathode = lPhotocathodeMaterial->GetMaterialPropertiesTable();
  
  pp = lMPT_photocathode->GetProperty(kABSLENGTH); //retrieves absorbtion length of the photocathode (could be redundant with if-function below)

  G4cout << "blah234" << G4endl;


  G4MaterialPropertiesTable* MPT = fMaterial2->GetMaterialPropertiesTable();
  if((pp = MPT->GetProperty(kRINDEX))) // checks if the given property exists
  {
    fRindex2 = pp->Value(fPhotonMomentum, idx_rindex2); //retrieves refractive index of Material2 at the given photon momentum
  }
  if ((pp = MPT->GetProperty(kABSLENGTH)))
  {
  fAbsorptionLength2 = pp->Value(fPhotonMomentum, idx_abslength2); // Retrieves absorption length of Material2 at the given photon momentum
  }
  ...
  if((pp = MPT->GetProperty(kCOATEDRINDEX)))
  {
    fCoatedRindex = pp->Value(fPhotonMomentum, idx_coatedrindex); //retrieves refractive index of the coating
  }
  if ((pp = MPT->GetProperty(kCOATEDABSLENGTH)))
  {
  fCoatedAbsLength = pp->Value(fPhotonMomentum, idx_coatedabslength); // Retrieves absorption length of the coating at the given photon momentum
  }
  ...
  //convert absorption length to complex refractive index: k(λ) = λ/( 4pi*abslength(λ) )
  fImagRIndex1      = wavelength / (4 * pi * fAbsorptionLength1); 
  fImagRIndex2      = wavelength / (4 * pi * fAbsorptionLength2);
  fCoatedImagRIndex = wavelength / (4 * pi * fAbsorptionLength1);
  ...
  do {
    ...
    if (random < (transCoeff + refCoeff)) {  // Photon is reflected
  ...
  } else if (random < transCoeff) { // Photon is transmitted
...
/* Old structure that did not consider absorption
    if (!G4BooleanRand(transCoeff)) {
...
OpticalLayerResult G4OpBoundaryProcess::GetFresnelThroughThinLayer(G4double sinTL,
  ...
  else //if (sinTL < 1.0)
  {
    ...
    if (cost1 != 0.0)
    {
      rTE = (r1toTL + rTLto2 * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL)) /
            (1.0 + r1toTL * rTLto2 * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL));
      tTE = (t1toTL_TE * t1toTL_TE * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL)) /
            (1.0 + r1toTL_TE * rTLto2_TE * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL));
    }

    // TM
    r1toTL = (fComplexRindex1 * costTL - fComplexCoatedRindex * cost1) / (fComplexRindex1 * costTL + fComplexCoatedRindex * cost1);
    rTLto2 = (fComplexCoatedRindex * cost2 - fComplexRindex2 * costTL) / (fComplexCoatedRindex * cost2 + fComplexRindex2 * costTL);
    r1toTL_TM = r1toTL;
    rTLto2_TM = rTLto2;
    ...
    if (cost1 != 0.0)
    {
      rTM = (r1toTL + rTLto2 * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL)) /
            (1.0 + r1toTL * rTLto2 * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL));
      tTM = (t1toTL_TM * t1toTL_TM * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL)) /
            (1.0 + r1toTL_TM * rTLto2_TM * std::exp(2.0 * i * k0 * fComplexCoatedRindex * fCoatedThickness * costTL));
  ...
  Trans_TE = (fRindex2 * cost2) / (fRindex1 * cost1) * tTE * tTE;
  Trans_TM = (fRindex2 * cost2) / (fRindex1 * cost1) * tTM * tTM;
  Transmittance = (Trans_TE + Trans_TM) / 2.0;

  Abs = 1.0 - Reflectivity - Transmittance;
